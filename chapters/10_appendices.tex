...
в \texttt{TESTS.md}
(09.04.2026, commit \texttt{c76dbafa2e2f93f72d2ceae4eadd7f7de78ac4c3}).
Перегенерация отчёта выполняется скриптом \texttt{regenerate\_tests\_md.sh}.
...
Отдельно выделены transport/service-тесты,
которые поднимают реальный gRPC-сервер на loopback,
а также smoke-тесты backend-части Tauri
без запуска GUI-диалогов.
...
18 unit-тестов, 1 интеграционный CSV-тест и 1 doctest.
...
включая проверки batch-валидации, health-check и backpressure.
Для backend-части Tauri
подтверждены smoke-тесты
...
запускаются отдельно и оформляются в отчёте как Bench-секции.
...
\item \texttt{IF97\_KERNEL} --- выбор режима вычислительного ядра (\texttt{core} или \texttt{stub}).  
\end{itemize}
